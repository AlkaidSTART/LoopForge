...
- 优先使用 Codex 已发现的 `$knowledge-distillation` skill。
- 如果当前会话没有注入该 skill，读取
  `.codex/skills/knowledge-distillation/SKILL.md` 并完整执行其中
  `DevFlow Override`。
- 只有 `.codex/skills/knowledge-distillation/SKILL.md` 缺失或不可读时，才允许
...
完成后：
```
返回主 Codex 线程：event=TASK-05_completed，summary="TASK-05_completed"
```
