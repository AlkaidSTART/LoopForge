...
主 Cursor 线程作为 orchestrator，通过 `workflow-state.json` 驱动阶段流转。

## Step 0：读取配置

读取：

1. `.cursor/assets/devflow.defaults.yaml`
2. `.cursor/runtime/workflow-state-spec.md`
3. `.cursor/rules/global.mdc`

可选读取 `.cursor/workflows/devflow.md` 做恢复检查。禁止在运行期读取
`.cursor/` 作为 Cursor 工作流来源。
...
3. 创建阶段目录：
   ...
   - `05-knowledge/`
   - `01-solo/`
4. 从 `.cursor/assets/workflow-state-template.json` 创建 `workflow-state.json`。
...
主 Cursor 线程直接执行，禁止委托给 subagent，因为该阶段需要和用户多轮对话。

0. 代码调研委托 explorer 只读 agent（结果不进主线程上下文）⚡⭐：
   - 主线程**不亲自**大规模 read/search 代码库，而是通过 `Task`
...
1. 必须调用或显式使用 `superpowers:brainstorming` skill：
   - 优先使用 Cursor 已发现的 `$superpowers:brainstorming` /
     `$superpowers-brainstorming` skill。
   - 如果当前会话没有注入该 skill，读取
     `.cursor/skills/superpowers/brainstorming/SKILL.md` 并完整执行其中
     `DevFlow Override`。
...
## Step 4：Cursor multi-agent 派发

TASK-02 及之后的角色阶段必须通过 Cursor `Task`
派发。主 Cursor 线程只负责准备上下文、派发任务、接收结果、合并产物和更新
`workflow-state.json`。

### 4.1 派发规则

| 阶段 | Cursor agent type | 角色文件 | 产物 |
|------|------------------|----------|------|
| TASK-02 | explorer | `.cursor/agents/architect.md` | `02-design/tech-design.md`, `02-design/execution-plan.md` |
| TASK-03 | worker | `.cursor/agents/developer.md` | `03-code/change-report.md`, `03-code/api-docs.md` |
| CODE-REVIEW | explorer | `.cursor/agents/code-reviewer.md` | `03-code/code-review-report.md` |
| TASK-04 | explorer | `.cursor/agents/test-engineer.md` | `04-e2e/test-report.md` |
| TASK-05 | explorer | `.cursor/agents/knowledge-engineer.md` | `05-knowledge/knowledge-report.md` |
| final summary（仅 medium/large） | explorer | `.cursor/agents/leader.md` | `workflow-summary.md` |
...
2. 当前阶段名称和目标产物路径。
3. 对应 `.cursor/agents/<role>.md` 的职责摘要。
4. 必要的 `.cursor/rules/` 和 checklist。
...
每次 `Task` 成功后，主线程必须在 `workflow-state.json.decisions[]`
追加记录：
...
```json
{
  ...
  "stage": "TASK-02",
  "role": "architect",
  "reason": "Task not available in this session"
}
```
...
7. 主线程按 `.cursor/assets/devflow.defaults.yaml.routing_table` 结合阶段报告中的
