---
name: devflow-knowledge-engineer
...
调用或显式使用 `knowledge-distillation` skill — **不得用手写替代**：

- 优先使用 Claude Code 已发现的 `$knowledge-distillation` skill。
- 如果当前会话没有注入该 skill，读取
  `.claude/skills/knowledge-distillation/SKILL.md` 并完整执行其中
  `DevFlow Override`。
- 只有 `.claude/skills/knowledge-distillation/SKILL.md` 缺失或不可读时，才允许
  标记 TASK-05 failed；不得在缺少 skill 指令时手写替代。
...
完成后：
```
返回主 Claude Code 线程：event=TASK-05_completed，summary="TASK-05_completed"
```
