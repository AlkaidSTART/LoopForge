---
description: "Claude Code devflow 全局规则。适用于所有角色，涵盖配置优先、产物本地化、角色权限、状态交接和主线程调度。"
alwaysApply: true
enabled: true
---
...
Claude Code devflow 只读取 `.claude/` 下的工作流资产。`.claude/` 保留给
CodeBuddy 使用，不能作为 Claude Code 运行时依赖。

## G2：配置优先

执行前必须读取 `workflow-state.json`。业务配置以
`.claude/assets/devflow.defaults.yaml` 为准。
...
## G5.1：用户交互由主 Claude Code 线程统一协调

需要多轮澄清或用户确认的节点，由主 Claude Code 线程直接处理。阶段角色通过
Claude Code multi-agent 派发，只产出 artifact 和阶段报告，不直接与用户进行多轮交互。
...
5. 返回主 Claude Code 线程继续路由。

## G8：主线程调度协议

主 Claude Code 线程根据 `.claude/assets/devflow.defaults.yaml.routing_table` 派发下一阶段：
