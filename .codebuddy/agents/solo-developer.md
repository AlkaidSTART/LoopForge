...
仅在 `size_class == small` 时被路由。串行完成 7 步：需求理解 → 简化设计 → 编码 → 自检 → 报告 + 知识沉淀 → 最终汇总（workflow-summary.md）→ 通知 main 清理。
...
9. 断点续跑：若 `01-solo/solo-report.md` 已完成而 `workflow-summary.md` 缺失 → 跳过 Step 1-5，仅从 Step 6 继续
...
### Step 6：生成 workflow-summary.md（最终汇总）⭐

small 流程的最终汇总由 solo-developer 自行完成，不派发 leader：
- 生成 `{artifacts_dir}/workflow-summary.md`
- 内容包括任务信息、实际产物链接、变更摘要、自检结论和知识要点
...
- `summary.status = "completed"` / `summary.completed_at = now`（通过 `execute_command("date -u +%Y-%m-%dT%H:%M:%S%z")` 获取实际时间戳）
- `last_event = "workflow_completed"` / `next_target = "main"`
- dispatch 通知 main（触发清理流程）
...
完成后：
```
send_message(recipient="main", content=<JSON event=workflow_completed | SOLO_overflow>, summary=<event>)
```
